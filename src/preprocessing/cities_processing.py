...
def process_macro_flows(
    ...
    dad_edges_final["migra"] = dad_edges_final.apply(
        lambda x: float(x["pop21"]) / x["pop"], axis=1
    )
    ...
    dt_edges_final["d_t"] = dt_edges_final.apply(
        lambda x: float(x["pop21"]) / x["pop"], axis=1
    )
    dt_edges_final = dt_edges_final[["code", "code_a", "d_t"]]

    dt_edges_final = dt_edges_final.loc[
        ~dt_edges_final["code_a"].isin(["YYYYY", "99999"])
    ].copy()
    dad_edges_final = dad_edges_final.loc[
        ~dad_edges_final["code_a"].isin(["YYYYY", "99999"])
    ].copy()
    edges_final = dt_edges_final.merge(
        dad_edges_final, how="outer", on=["code", "code_a"]
    ).fillna(0)
